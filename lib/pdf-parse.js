...
async function PDF(dataBuffer, options) {
    ...
    if (typeof options.textWithPageNum != 'boolean') options.version = DEFAULT_OPTIONS.textWithPageNum;
    ...
    for (var i = 1; i <= counter; i++) {
      ...
      const textWithPageNum = {
        ...
        ret.text = `${ret.text}\n\n${pageText}`;
        if (options.textWithPageNum) {
          textWithPageNum.text = pageText
          textWithPageNum.pageNum = i
          ret.textWithPageNum.push(textWithPageNum)
        }
    }
